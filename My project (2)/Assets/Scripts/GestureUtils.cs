...
// This method is used to bring up the menu item on the left hand
        public static bool IsShowWatchGestureDetected(Handedness trackedHand){
            return (HandPoseUtils.IndexFingerCurl(trackedHand) > PinchThreshold) &&
                   (HandPoseUtils.MiddleFingerCurl(trackedHand)  > PinchThreshold)&&  
                   (HandPoseUtils.RingFingerCurl(trackedHand)  > PinchThreshold)&&
                   (HandPoseUtils.PinkyFingerCurl(trackedHand)   > PinchThreshold)&&
                   (HandPoseUtils.ThumbFingerCurl(trackedHand)  > PinchThreshold);
        }
    }